...
{
  return d->tag.access<ID3v2::Tag>(WavFile_Helper::ID3v2Index, false);
}

RIFF::Info::Tag *RIFF::WAV::File::InfoTag() const
{
  return d->tag.access<RIFF::Info::Tag>(WavFile_Helper::InfoIndex, false);
}
...
bool RIFF::WAV::File::save(TagTypes tags, bool stripOthers, int id3v2Version)
{
  ...
  ID3v2::Tag *id3v2tag = d->tag.access<ID3v2::Tag>(WavFile_Helper::ID3v2Index, false);
  if(!id3v2tag->isEmpty()) {
    if(tags & ID3v2) {
  ...
  Info::Tag *infotag = d->tag.access<Info::Tag>(WavFile_Helper::InfoIndex, false);
  if(!infotag->isEmpty()) {
    if(tags & Info) {
...
void RIFF::WAV::File::read(bool readProperties, Properties::ReadStyle propertiesStyle)
{
  ...
  for(uint i = 0; i < chunkCount(); i++) {
    ...
    if(name == "ID3 " || name == "id3 ") {
      ...
      d->tag.set(WavFile_Helper::ID3v2Index, new ID3v2::Tag(this, chunkOffset(i)));
      d->hasID3v2 = true;
    }
      ...
      if(type == "INFO") {
        d->tag.set(WavFile_Helper::InfoIndex, new RIFF::Info::Tag(data));
        d->hasInfo = true;
      }
    }
  }

  if (!d->tag[WavFile_Helper::ID3v2Index])
    d->tag.set(WavFile_Helper::ID3v2Index, new ID3v2::Tag);

  if (!d->tag[WavFile_Helper::InfoIndex])
    d->tag.set(WavFile_Helper::InfoIndex, new RIFF::Info::Tag);
