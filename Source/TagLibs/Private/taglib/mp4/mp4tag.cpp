...
static const char *Mp4TagkeyTranslation[][2] = {
  { "\251nam", "TITLE" },
  { "\251ART", "ARTIST" },
  { "\251alb", "ALBUM" },
  { "\251cmt", "COMMENT" },
...
PropertyMap MP4::Tag::properties() const
{
  ...
  if(keyMap.isEmpty()) {
    int numKeys = sizeof(Mp4TagkeyTranslation) / sizeof(Mp4TagkeyTranslation[0]);
    for(int i = 0; i < numKeys; i++) {
      keyMap[Mp4TagkeyTranslation[i][0]] = Mp4TagkeyTranslation[i][1];
    }
  }
...
PropertyMap MP4::Tag::setProperties(const PropertyMap &props)
{
  ...
  if(reverseKeyMap.isEmpty()) {
    int numKeys = sizeof(Mp4TagkeyTranslation) / sizeof(Mp4TagkeyTranslation[0]);
    for(int i = 0; i < numKeys; i++) {
      reverseKeyMap[Mp4TagkeyTranslation[i][1]] = Mp4TagkeyTranslation[i][0];
    }
  }
