...
public:
  FileStreamPrivate(const FileName &fileName)
    : file(TFileStream_Helper::InvalidFileHandle)
    , name(fileName)
    , readOnly(true)
  {
  }

  TFileStream_Helper::FileHandle file;
  TFileStream_Helper::FileNameHandle name;
  bool readOnly;
};
  ...
  : d(new FileStreamPrivate(fileName))
{
  ...
  if(!openReadOnly)
    d->file = TFileStream_Helper::openFile(fileName, false);

  if(d->file != TFileStream_Helper::InvalidFileHandle)
    d->readOnly = false;
  else
    d->file = TFileStream_Helper::openFile(fileName, true);
...
{
  if(isOpen())
	  TFileStream_Helper::closeFile(d->file);
...
ByteVector FileStream::readBlock(ulong length)
{
  ...
  const size_t count = TFileStream_Helper::readFile(d->file, buffer);
...
void FileStream::writeBlock(const ByteVector &data)
{
  ...
  TFileStream_Helper::writeFile(d->file, data);
}
...
void FileStream::insert(const ByteVector &data, ulong start, ulong replace)
{
  ...
  while(true)
  {
    ...
    seek(readPosition);
    const size_t bytesRead = TFileStream_Helper::readFile(d->file, aboutToOverwrite);
...
void FileStream::removeBlock(ulong start, ulong length)
{
  ...
  {
    seek(readPosition);
    bytesRead = TFileStream_Helper::readFile(d->file, buffer);
    ...
    seek(writePosition);
	TFileStream_Helper::writeFile(d->file, buffer);
...
{
  return (d->file != TFileStream_Helper::InvalidFileHandle);
}
...
{
  return TFileStream_Helper::BufferSize;
}
