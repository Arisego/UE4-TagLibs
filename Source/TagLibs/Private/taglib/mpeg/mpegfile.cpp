...
PropertyMap MPEG::File::properties() const
{
  ...
  // within TagUnion.
  if(d->hasID3v2)
    return d->tag.access<ID3v2::Tag>(MpegFile_Helper::ID3v2Index, false)->properties();
  if(d->hasAPE)
    return d->tag.access<APE::Tag>(MpegFile_Helper::APEIndex, false)->properties();
  if(d->hasID3v1)
    return d->tag.access<ID3v1::Tag>(MpegFile_Helper::ID3v1Index, false)->properties();
  return PropertyMap();
}
...
{
  if(d->hasID3v2)
    d->tag.access<ID3v2::Tag>(MpegFile_Helper::ID3v2Index, false)->removeUnsupportedProperties(properties);
  else if(d->hasAPE)
    d->tag.access<APE::Tag>(MpegFile_Helper::APEIndex, false)->removeUnsupportedProperties(properties);
  else if(d->hasID3v1)
    d->tag.access<ID3v1::Tag>(MpegFile_Helper::ID3v1Index, false)->removeUnsupportedProperties(properties);
}
...
PropertyMap MPEG::File::setProperties(const PropertyMap &properties)
{
  if(d->hasID3v1)
    ...
    d->tag.access<ID3v1::Tag>(MpegFile_Helper::ID3v1Index, false)->setProperties(properties);
  return d->tag.access<ID3v2::Tag>(MpegFile_Helper::ID3v2Index, true)->setProperties(properties);
}
...
bool MPEG::File::save(int tags, bool stripOthers, int id3v2Version, bool duplicateTags)
{
  ...
  if (duplicateTags) {
    ...
    if((tags & ID3v1) && d->tag[MpegFile_Helper::ID3v2Index] && !(stripOthers && !(tags & ID3v2)))
  ...
  if((APE & tags) && APETag()) {
    if(d->hasAPE)
      ...
      if(d->hasID3v1) {
        ...
        seek(0, End);
        d->APELocation = tell();
        APE::Tag *apeTag = d->tag.access<APE::Tag>(MpegFile_Helper::APEIndex, false);
...
{
  return d->tag.access<ID3v2::Tag>(MpegFile_Helper::ID3v2Index, create);
}

ID3v1::Tag *MPEG::File::ID3v1Tag(bool create)
{
  return d->tag.access<ID3v1::Tag>(MpegFile_Helper::ID3v1Index, create);
}

APE::Tag *MPEG::File::APETag(bool create)
{
  return d->tag.access<APE::Tag>(MpegFile_Helper::APEIndex, create);
}
...
bool MPEG::File::strip(int tags, bool freeMemory)
{
  ...
  if((tags & ID3v2) && d->hasID3v2) {
    ...
    if(freeMemory)
      d->tag.set(MpegFile_Helper::ID3v2Index, 0);
  ...
  if((tags & ID3v1) && d->hasID3v1) {
    ...
    if(freeMemory)
      d->tag.set(MpegFile_Helper::ID3v1Index, 0);
  }
  ...
  if((tags & APE) && d->hasAPE) {
    ...
    if(freeMemory)
      d->tag.set(MpegFile_Helper::APEIndex, 0);
  }
...
void MPEG::File::read(bool readProperties, Properties::ReadStyle propertiesStyle)
{
  ...
  if(d->ID3v2Location >= 0) {
    ...
    d->tag.set(MpegFile_Helper::ID3v2Index, new ID3v2::Tag(this, d->ID3v2Location, d->ID3v2FrameFactory));
  ...
  if(d->ID3v1Location >= 0) {
    d->tag.set(MpegFile_Helper::ID3v1Index, new ID3v1::Tag(this, d->ID3v1Location));
    d->hasID3v1 = true;
  }
  ...
  if(d->APELocation >= 0) {
    ...
    d->tag.set(MpegFile_Helper::APEIndex, new APE::Tag(this, d->APEFooterLocation));
    d->APEOriginalSize = APETag()->footer()->completeTagSize();
    d->hasAPE = true;
  }
