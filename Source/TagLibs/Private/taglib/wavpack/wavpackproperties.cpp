...
void WavPack::Properties::read()
{
  ...
  const unsigned int flags = d->data.toUInt(24, (bool)false);
  ...
  unsigned int samples = d->data.toUInt(12, (bool)false);
  if(samples == ~0u) {
...
unsigned int WavPack::Properties::seekFinalIndex()
{
  ...
  while(offset > 0) {
    ...
    if(version < MIN_STREAM_VERS || version > MAX_STREAM_VERS)
      continue;
    const unsigned int flags = data.toUInt(24, (bool)false);
    if(!(flags & FINAL_BLOCK))
      return 0;
    const unsigned int blockIndex   = data.toUInt(16, (bool)false);
    const unsigned int blockSamples = data.toUInt(20, (bool)false);
